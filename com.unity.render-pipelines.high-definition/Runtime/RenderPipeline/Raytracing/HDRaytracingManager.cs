            ...
            bool pathTracingEnabled, int ptLayerValue)
        {
            ...
            if (materialArray != null)
            {
                ...
                if (aoEnabled && !materialIsTransparent)
                {
                    ...
                    instanceFlag |= ((aoLayerValue & objectLayerValue) != 0) ? (uint)(1 << 3) : 0x00;
                }

                if (reflEnabled && !materialIsTransparent)
                {
                    // Raise the Screen Space Reflection if needed
                    instanceFlag |= ((reflLayerValue & objectLayerValue) != 0) ? (uint)(1 << 4) : 0x00;
                }

                if (giEnabled && !materialIsTransparent)
                {
                    // Raise the Global Illumination if needed
                    instanceFlag |= ((giLayerValue & objectLayerValue) != 0) ? (uint)(1 << 5) : 0x00;
                }

                if (recursiveEnabled)
                {
                    // Raise the Global Illumination if needed
                    instanceFlag |= ((rrLayerValue & objectLayerValue) != 0) ? (uint)(1 << 6) : 0x00;
                }

                if (pathTracingEnabled)
                {
                    // Raise the Global Illumination if needed
                    instanceFlag |= ((ptLayerValue & objectLayerValue) != 0) ? (uint)(1 << 7) : 0x00;
                }
